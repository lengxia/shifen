...
public class DateDeserializer extends JsonDeserializer<Object> {
    ...
    public Object deserialize(JsonParser p, DeserializationContext ctxt)
        ...
        try {
            ...
            if (tmp.length() == 13) {
                return new Date(p.getValueAsLong()).toInstant().atOffset(ZoneOffset.of("+8")).toLocalDateTime();
            } else if (tmp.length() == 10) {
                return new Date(p.getValueAsLong() * 1000).toInstant().atOffset(ZoneOffset.of("+8")).toLocalDateTime();
            } else
                return null;
        } catch (Exception e) {
